          ...
          <div class="settings-card">
            ...
            <form class="settings-form">
              <div class="form-group">
                ...
                <input v-model="settings.maxLoginAttempts" type="number" min="3" max="10" />
              </div>
              
              <div class="form-group">
                <label>Durée de session (minutes)</label>
                <input v-model="settings.sessionDuration" type="number" min="30" max="1440" />
              </div>
...
const deleteCategory = async (id: string) => {
  ...
  const confirmed = await confirm({
    ...
    message: `Êtes-vous sûr de vouloir supprimer la catégorie "${category?.name}" ? Tous les médias associés devront être reclassés.`,
    type: 'danger',
